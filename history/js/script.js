				...
				success: function( jsonData ) {
					...
					if(jsonData["ok"] == 0)
					{
						...
						if ((target == 'month') || (target == 'year')){
								for(var i=0; i<data.length; i++) { data[i] = parseFloat(data[i], 10)*0.024; } 
						}
						else{
						...
						if ((target == 'month') || (target == 'year')){
								for(var i=0; i<data2.length; i++) { data2[i] = parseFloat(data2[i], 10)*0.024; } 
						}
						else{
						...
						if ((target !== 'week') && (target !== 'month') && (target !== 'year')){			//DUAL BARS | LIVE DAY
						...
						}else if (target == 'week'){			//DUAL BARS | WEEK
						...
						historychart = new Highcharts.StockChart({
							...
							//colors: ['#C4DBF5', '#9DC5F2', '#5D9EE8', '#2f7ed8'],																		//COLORS 		| Blue + lighter blue + lighter blue + lighter blue						
							//colors: ['#5D9EE8', '#2f7ed8'],																		//COLORS 		| Blue + lighter blue							
							//colors: ['#4572A7', '#AA4643', '#89A54E', '#80699B', '#3D96AE', '#DB843D', '#92A8CD', '#A47D7C', '#B5CA92'],				//COLORS 		| 'Orig Highstocks' V2.x Colors
