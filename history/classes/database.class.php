...
class Database {
    ...
    public function updateMeterc($time, $count, $islow) {
        try {
			$sth = $this->_db->prepare("INSERT INTO meter (`time`,`count`,`islow`) VALUES (:time, :count, :islow) ON DUPLICATE KEY UPDATE `time`=:time, `count`=:count, `islow`=:islow");
     ...
     */
     public function getMetercal() {
        try {
            $sth = $this->_db->prepare("SELECT * FROM meter");
	...
	* Get Feestdagen 
     */
     public function getFeestdagen() {
        try {
            $sth = $this->_db->prepare("SELECT * FROM feestdagen");
	...
	public function getMeterstand($IsLow) {
		try {
            $sth = $this->_db->prepare("select sum(value) as stand from (
											...
											where IsLow = :IsLow
											and time > (
												SELECT max(time) FROM `meter` b WHERE islow = :IsLow)
											union 
											select count as value 
											from meter 
											where time=(
												select max(time) from meter where IsLow= :IsLow) 
											and IsLow= :IsLow) e");
    ...
    */ 
    public function addMinuteData($time, $unit, $delta, $values, $islow, $cpkwh) {
        ...
        try {
            $sth = $this->_db->prepare("INSERT INTO data_m (
            	time,
				unit,
				delta,
				value,
	    ...
	    ON DUPLICATE KEY UPDATE time = :time, value = :value, delta = :delta, unit = :unit, cpkwh = :cpkwh, IsLow = :islow");
...
");
    ...
    */ 
    public function addMissingMinuteData($time) {
        ...
        try {
            $sth = $this->_db->prepare("INSERT INTO data_m (   time,
				unit,
				delta,
				value,
				cpKwh,
